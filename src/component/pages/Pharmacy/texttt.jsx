...
export default function DialogCreateRec(treatmentData) {
    ...
    const [inputs, setInputs] = useState([
      {"value1": "ค่าตรวจจากห้องปฏิบัติการ", "value2": 130},
      {"value1": "ค่าตรวจอื่นๆ (EKG/X-ray)", "value2": 140},
      {"value1": "ค่ายา/เวชภัณฑ์/อุปกรณ์", "value2": 150},
      {"value1": "ค่าบริการทางการแพทย์", "value2": 100}
    ]);
    ...
    useEffect(() => {
      ...
      if (treatmentData?.treatmentData?.rx?.length > 0) {
        RxItems = RxItems.concat(pushData(treatmentData.treatmentData.rx, "medicine_name", "total_price"));
      }
  
      if (treatmentData?.treatmentData?.lab_results?.length > 0) {
        LabItems = LabItems.concat(pushData(treatmentData.treatmentData.lab_results, "test_name", "total_price"));
      }
  
      if (treatmentData?.treatmentData?.xr_results?.length > 0) {
        PXItems = PXItems.concat(pushData(treatmentData.treatmentData.xr_results, "description", "total_price"));
      }
  
      if (treatmentData?.treatmentData?.prx?.length > 0) {
        PXItems = PXItems.concat(pushData(treatmentData.treatmentData.prx, "procedure_name", "total_price"));
      }
  
      if (treatmentData?.treatmentData?.pe?.length > 0) {
        PeItems = PeItems.concat(pushData(treatmentData.treatmentData.pe, "description", "total_price"));
      }
  
      setInputs(allInputs);

      const LabSum = LabItems.reduce((sum, item) => {
        const price = parseFloat(item.value2.toString().replace(/,/g, "")) || 0;
        return sum + price;
      }, 0);
      setLabTotal(LabSum);
      const PXSum = PXItems.reduce((sum, item) => {
        const price = parseFloat(item.value2.toString().replace(/,/g, "")) || 0;
        return sum + price;
      }, 0);
      setPXTotal(PXSum);
      const RxSum = RxItems.reduce((sum, item) => {
        const price = parseFloat(item.value2.toString().replace(/,/g, "")) || 0;
        return sum + price;
      }, 0);
      setRxTotal(RxSum);
      const PeSum = PeItems.reduce((sum, item) => {
        const price = parseFloat(item.value2.toString().replace(/,/g, "")) || 0;
        return sum + price;
      }, 0);
      setPeTotal(PeSum);
